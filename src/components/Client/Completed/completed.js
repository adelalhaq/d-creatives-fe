...
function Completed(props) {
  ...
  const localStats = clientStatsJSON ? JSON.parse(clientStatsJSON) : null;
  ...
  const { data: statsClient } = useSelector((state) => state.getStatsClient);
  ...
  const filteredProjects = useMemo(() => {
    if (companyId) {
      ...
      filtered = filtered.filter(
        (item) => item.completed === true && !item.hide
      );
      if (companyId) {
        filtered = filtered?.filter(
        ...
        {data
          ...
          .map((project, index) => (
            ...
            >
              ...
              <div className="flex flex-col justify-center items-center lg:col-span-3 col-span-12">
                <a
                  className="flex  justify-center items-center  p-1 m-1 project-btn-01"
                  target="_blank"
                  href={project.talentFootageLink}
                >
                  <img src={project03} alt="logo-1" className="w-5 me-2" />
                  <div>Raw Files</div>
                </a>

                <a
                  className="flex  justify-center items-center  p-1 m-1 project-btn-03"
                  target="_blank"
                  href={project.briefLink}
                >
                  <img src={project01} alt="logo-1" className="w-5 me-2" />
                  <div>Brief Link</div>
                </a>

                {/* <a
                      className="flex flex-col justify-center items-center p-1 m-1"
                      target="_blank"
  ...
  return (
    <div>
      <div className="sm:p-8 p-4 sm:ml-64 mt-16">
        ...
        <div className="dashboard-heading mb-5">Completed Projects</div>
